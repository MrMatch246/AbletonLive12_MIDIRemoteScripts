...
class ChannelStripController(MackieControlComponent):
    ...
    def handle_assignment_switch_idsParse error at or near `JUMP_FORWARD' instruction at offset 236_238

    def handle_vpot_rotationParse error at or near `COME_FROM' instruction at offset 204_2
            ...
            if self._ChannelStripController__assignment_mode == CSM_IO:
                if s.assigned_track():
                    if s.assigned_track().has_audio_output:
                        slider_param = (
                         s.assigned_track().mixer_device.volume, "Volume")
                    if self._ChannelStripController__flip:
